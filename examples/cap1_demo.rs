...
fn create_medical_cap0(
    ...
    let _signing_key_entry = _public_keys.signing_key()
        .ok_or_else(|| capsula_core::error::CoreError::DataError("No signing key found".to_string()))?;
    ...
    let cap0_capsule = CapsuleBuilder::new()
        ...
        .with_text_annotation(serde_json::to_string(&serde_json::json!({
            "type": "医疗影像描述",
    ...
    let cap0 = if let CapsulePayload::Cap0(cap0) = cap0_capsule.get_payload() {
        ...
        return Err(capsula_core::error::CoreError::DataError("载荷类型不匹配".to_string()));
    };
...
fn create_cap1_with_bnf_extract(
    ...
    let cap1_capsule = CapsuleBuilder::new()
        ...
        .with_meta_data(serde_json::to_vec(&serde_json::json!({
            "processing_info": {
...
fn display_capsule_summary(capsule: &Capsule) {
    ...
    println!("    🖼️  包含水印: {}", if summary.has_watermark { "是" } else { "否" });
    println!("    📝 包含审计: {}", if summary.has_audit_ref { "是" } else { "否" });
