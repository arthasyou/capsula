...
mod tests {
    ...
    fn create_test_cap0() -> crate::Result<Cap0> {
        // 创建测试用的密钥和数据
        ...
        let signing_key_entry = public_keys.signing_key().ok_or_else(|| {
            crate::error::CoreError::DataError("No signing key found".to_string())
        })?;
        let recipient_public_key_spki = signing_key_entry.spki_der.clone();

        // 创建原始数据封装块
        let origin_data = b"Test medical image data";
        ...
        Ok(Cap0::new(origin_block, None))
    }

    #[test]
    fn test_capsule_creation() -> crate::Result<()> {
        // 创建测试数据
        let cap0 = create_test_cap0()?;

        let header = CapsuleHeader::new(
            "cid:test123".to_string(),
            "1.0".to_string(),
            CapsulaStage::First,
        ...
        // 验证基本属性
        assert_eq!(capsule.get_id(), "cid:test123");
        assert_eq!(capsule.get_payload_type(), "Cap0");
        assert!(matches!(capsule.get_stage(), CapsulaStage::First));

        // 验证AAD绑定
        assert!(capsule.verify_aad_binding()?);

        Ok(())
    }

    #[test]
    fn test_aad_computation() -> crate::Result<()> {
        let cap0 = create_test_cap0()?;

        let header = CapsuleHeader::new(
            "cid:test456".to_string(),
            "1.0".to_string(),
            CapsulaStage::First,
            "medical.test".to_string(),
            None,
    ...
    #[test]
    fn test_capsule_summary() -> crate::Result<()> {
        let cap0 = create_test_cap0()?;

        let mut header = CapsuleHeader::new(
            "cid:test789".to_string(),
            "1.0".to_string(),
            CapsulaStage::First,
            "medical.test".to_string(),
            Some("test_creator".to_string()),
        );
        header.add_metadata("department".to_string(), "radiology".to_string());

        let policy = PolicyControl::new(
            "policy_ref_789".to_string(),
            vec!["read".to_string(), "write".to_string()],
        );
        ...
        let mut capsule = Capsule::new(header, policy, keyring, payload)?;

        // 添加水印和审计引用
        let watermark = Watermark::new(
            "invisible".to_string(),
            "watermark_data".to_string(),
            "LSB".to_string(),
        );
        capsule.add_watermark(watermark);
        capsule.set_audit_ref("audit_ref_123".to_string());

        // 验证基本属性
        assert_eq!(capsule.header.id, "cid:test789");
        assert!(capsule.integrity.watermark.is_some());
        assert!(capsule.audit_ref.is_some());

        Ok(())
    }

    #[test]
    fn test_capsule_serialization() -> crate::Result<()> {
        let cap0 = create_test_cap0()?;

        let header = CapsuleHeader::new(
            "cid:serial_test".to_string(),
            "1.0".to_string(),
            CapsulaStage::First,
            "medical.test".to_string(),
            None,
        ...
        let deserialized: Capsule = serde_json::from_str(&json).unwrap();

        // 验证反序列化后的数据
        assert_eq!(capsule.get_id(), deserialized.get_id());
        assert_eq!(capsule.get_payload_type(), deserialized.get_payload_type());
        // 基本序列化验证 - AadBinding 比较需要实现 PartialEq
        assert_eq!(capsule.header.id, deserialized.header.id);

        Ok(())
    }

    #[test]
    fn test_watermark_functionality() {
        let watermark = Watermark::new(
            "visible".to_string(),
        ...
        assert_eq!(watermark.watermark_type, "visible");
        assert_eq!(watermark.data, "Copyright 2025");
        assert_eq!(watermark.algorithm, "DCT");
        assert!(!watermark.embedded_at.is_empty());
    }
}