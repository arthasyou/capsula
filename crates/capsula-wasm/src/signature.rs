...
pub fn sign_with_extended_info(
    ...
    let extended_info = ExtendedSignatureInfo {
        ...
        timestamp: (Date::now() / 1000.0) as u64,  // JavaScript Date.now() 返回毫秒，除以 1000 得到秒
...
fn verify_signature_internal(data: &[u8], digital_signature: &DigitalSignature) -> Result<bool, JsError> {
    // 计算原始数据的哈希
    let mut hasher = Sha256::new();
    hasher.update(data);
