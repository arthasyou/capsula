...
use crate::models::capsule::{
    EncapsulateRequest, EncapsulateResponse, DecapsulateRequest, 
    DecapsulateResponse, CapsuleVerifyRequest, CapsuleVerifyResponse
};
use crate::error::AppError;

use capsula_core::{CapsulaApi, Capsula1};
use capsula_key::{RsaKey, P256Key, Key, ExportablePrivateKey};
...
async fn simple_encapsulate_data(request: &EncapsulateRequest) -> Result<EncapsulateResponse, Box<dyn std::error::Error>> {
    ...
    let recipients = vec![
        (request.recipient_id.clone(), &recipient_key as &dyn Key)
    ];
...
async fn simple_decapsulate_data(request: &DecapsulateRequest) -> Result<DecapsulateResponse, Box<dyn std::error::Error>> {
    ...
    let private_key_der = general_purpose::STANDARD.decode(&request.private_key)
        .map_err(|e| {
            tracing::error!("Failed to decode base64 private key: {}", e);
            e
        })?;
    let private_key = RsaKey::from_pkcs8_der(&private_key_der)
        .map_err(|e| {
...
pub async fn verify_capsule(
    ...
    match simple_verify_capsule(&request).await {
        ...
        Err(e) => {
            ...
            Err(AppError::Internal("Capsule verification failed".to_string()))
        }
    }
}

/// Simple capsule verification implementation
async fn simple_verify_capsule(request: &CapsuleVerifyRequest) -> Result<CapsuleVerifyResponse, Box<dyn std::error::Error>> {
    ...
    Ok(CapsuleVerifyResponse {
        ...
        producer: capsule.meta.producer,
        owner: capsule.meta.owner,
        created_at: chrono::Utc::now(), // Placeholder - actual creation time would be from capsule metadata
