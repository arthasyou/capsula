...
use crate::{
    ...
    models::certificate::{
        ...
        CertificateRequest, CertificateResponse, CertificateSummary, RenewalRequest,
        RevocationRequest, UserCertificateListResponse, UserCertificateQuery,
    },
    state::AppState,
};
...
pub async fn create_certificate(
    ...
    use crate::certificate::{CertificateRequest as SignerRequest, CertificateSigner};
        ...
        .await?
    {
        return Err(AppError::BadRequest(format!(
            "User {} already has an active {} certificate (ID: {}). Please revoke the existing \
             certificate first.",
            request.username, algorithm_string, existing_cert.certificate_id
        )));
    }
...
pub async fn revoke_certificate(
    ...
    let reason_str = serde_json::to_string(&request.reason)
        ...
        .trim_matches('"')
        .to_string(); // Remove quotes from JSON string

    cert_service
        .revoke_certificate(&certificate_id, Some(reason_str), request.comment)
        .await?;
...
fn convert_record_to_response(
    ...
    CertificateResponse {
        ...
        not_before: chrono::DateTime::<chrono::Utc>::from_timestamp(record.not_before, 0)
            .unwrap_or_default(),
        not_after: chrono::DateTime::<chrono::Utc>::from_timestamp(record.not_after, 0)
            .unwrap_or_default(),
        status: record.status,
        created_at: chrono::DateTime::<chrono::Utc>::from_timestamp(record.created_at, 0)
            .unwrap_or_default(),
    }
}
...
pub async fn renew_certificate(
    ...
    tracing::info!(
        "Certificate renewal requested for ID: {}",
    ...
    if !matches!(
        original_cert.status,
        crate::models::certificate::CertificateStatus::Active
    ) {
        return Err(AppError::BadRequest(
            "Cannot renew revoked or expired certificate".to_string(),
        ));
    }
    ...
    let issued_cert = signer
        .renew_certificate(&username, request.validity_days)
        .await?;

    // Mark the old certificate as superseded
    cert_service
        .supersede_certificate(&original_cert.certificate_id)
        .await?;
    ...
    tracing::info!(
        "Certificate renewed successfully for user: {}, new ID: {}",
        username,
        certificate_id
    );
