...
impl FileKeyStore {
    pub fn new(path: PathBuf, encryption_key: Option<Vec<u8>>) -> Result<Self> {
        ...
        let cipher = if let Some(key) = encryption_key {
            ...
            let cipher =
                ChaCha20Poly1305::new_from_slice(&key).map_err(Error::invalid_configuration)?;
            Some(cipher)
        } else {
            None
        };
    ...
    fn decrypt(&self, ciphertext: &[u8]) -> Result<Vec<u8>> {
        if let Some(cipher) = &self.cipher {
            ...
            cipher.decrypt(nonce, encrypted).map_err(Error::decryption)
        } else {
...
impl KeyStore for FileKeyStore {
    fn store_key(&self, metadata: KeyMetadata, pkcs8_der_bytes: Vec<u8>) -> Result<()> {
        ...
        let encrypted_material = self.encrypt(&pkcs8_der_bytes)?;
    ...
    fn get_key(&self, handle: KeyHandle) -> Result<(KeyMetadata, Vec<u8>)> {
        ...
        let metadata_json =
            std::fs::read_to_string(&metadata_path).map_err(|_| Error::key_not_found(handle))?;
        ...
        let encrypted_material =
            std::fs::read(&key_path).map_err(|_| Error::key_not_found(handle))?;
        let pkcs8_der_bytes = self.decrypt(&encrypted_material)?;

        Ok((metadata, pkcs8_der_bytes))
    }
...
mod tests {
    ...
    fn test_file_store() {
        ...
        let pkcs8_der_bytes = vec![1, 2, 3, 4, 5];
    ...
    fn test_file_store_with_encryption() {
        ...
        assert_ne!(raw_key, pkcs8_der_bytes); // Should be different due to encryption
