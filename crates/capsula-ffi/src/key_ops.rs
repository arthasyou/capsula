...
use capsula_key::{Curve25519, P256Key, RsaKey, ExportablePrivateKey};
...
pub extern "C" fn capsula_key_generate(algorithm: CapsulaAlgorithm) -> *mut CapsulaResult {
    match algorithm {
        CapsulaAlgorithm::Curve25519 => {
            match Curve25519::generate() {
                Ok(key) => {
        ...
        CapsulaAlgorithm::Rsa2048 => {
            match RsaKey::generate_2048() {
                Ok(key) => {
                    match key.to_pkcs8_der() {
                        Ok(private_key_der) => CapsulaResult::success_boxed(private_key_der),
                        Err(e) => CapsulaResult::error_boxed(
        ...
        CapsulaAlgorithm::P256 => {
            match P256Key::generate() {
                Ok(key) => {
                    match key.to_pkcs8_der() {
                        Ok(private_key_der) => CapsulaResult::success_boxed(private_key_der),
                        Err(e) => CapsulaResult::error_boxed(
...
#[no_mangle]
pub extern "C" fn capsula_key_import_from_file(
    ...
    let path_str = unsafe {
        match CStr::from_ptr(file_path).to_str() {
            Ok(s) => s,
            Err(_) => return CapsulaResult::error_boxed(
                CapsulaError::InvalidInput,
    ...
    let pem_content = match std::fs::read_to_string(path_str) {
        Ok(content) => content,
        Err(e) => return CapsulaResult::error_boxed(
...
pub extern "C" fn capsula_key_export_to_file(
    ...
    let algorithm = match crate::utils::detect_algorithm_from_der(key_slice) {
        Some(alg) => alg,
        None => return CapsulaResult::error_boxed(
            CapsulaError::ImportFailed,
    ...
    match algorithm {
        CapsulaAlgorithm::Curve25519 => {
            match Curve25519::from_pkcs8_der(key_slice) {
                Ok(key) => {
                    match key.to_pkcs8_pem() {
                        Ok(pem_content) => {
                            match std::fs::write(path_str, pem_content) {
        ...
        CapsulaAlgorithm::Rsa2048 => {
            match RsaKey::from_pkcs8_der(key_slice) {
                Ok(key) => {
                    match key.to_pkcs8_pem() {
                        Ok(pem_content) => {
                            match std::fs::write(path_str, pem_content) {
        ...
        CapsulaAlgorithm::P256 => {
            match P256Key::from_pkcs8_der(key_slice) {
                Ok(key) => {
                    match key.to_pkcs8_pem() {
                        Ok(pem_content) => {
                            match std::fs::write(path_str, pem_content) {
